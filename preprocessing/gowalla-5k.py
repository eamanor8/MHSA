...
def get_dataset(config):
    ...
    data_path = f"./small-dataset/test-valid_ids_gowalla.pk"
    with open(data_path, "wb") as handle:
        pickle.dump(final_valid_id, handle, protocol=pickle.HIGHEST_PROTOCOL)
    gowalla_afterUser.to_csv(f"./small-dataset/test-dataset_gowalla.csv", index=False)
    gowalla_loc.to_csv(f"./small-dataset/test-locations_gowalla.csv", index=False)
